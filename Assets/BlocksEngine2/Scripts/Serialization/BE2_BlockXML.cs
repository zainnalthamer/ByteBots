...
{
    // v2.13 - BE2_BlockXML refactored to include the OuterArea xml element
    public static class BE2_BlockXML
    {
        ...
        public static XElement SBlockToXElement(BE2_SerializableBlock serializableBlock)
        {
            ...
            XElement xOuterArea = SOuterAreaToXElement(serializableBlock.outerArea);
            xBlock.Add(xOuterArea);
        ...
        {
            XElement xOuterArea = new XElement("OuterArea");

            XElement xChildBlocks = new XElement("childBlocks");
            xOuterArea.Add(xChildBlocks);
            foreach (BE2_SerializableBlock sChildBlock in serializableOuterArea.childBlocks)
            {
                XElement xChildBlock = SBlockToXElement(sChildBlock);
                xChildBlocks.Add(xChildBlock);
            }
        ...
        public static BE2_SerializableBlock XElementToSBlock(XElement xBlock)
        {
            ...
            XElement xOuterArea = xBlock.Element("OuterArea");
            BE2_SerializableOuterArea sOuterArea = XElementToSOuterArea(xOuterArea);
            sBlock.outerArea = sOuterArea;
        ...
        {
            BE2_SerializableOuterArea sOuterArea = new BE2_SerializableOuterArea();

            IEnumerable<XElement> xChildBlocks = xOuterArea.Element("childBlocks").Elements("Block");
            foreach (XElement xChildBlock in xChildBlocks)
            {
                BE2_SerializableBlock sChildBlock = XElementToSBlock(xChildBlock);
                sOuterArea.childBlocks.Add(sChildBlock);
            }
